 ...
 * This function is designed to work with the HTML structure generated by the
 * custom `VariableNode` extension, which represents variables as spans
 * like: `<span data-name="VAR_NAME"></span>`.
 *
 ...
 * @returns An HTML string with variable placeholders replaced by their values.
 */
export function interpolateTiptapContent(
  ...
  // This regex looks for the specific span structure created by VariableNode.
  ...
  return htmlContent.replace(
    ...
    (_match, variableName) => {
      ...
      // If no value is provided, return the original `{{VAR_NAME}}` format
      // for clarity in the preview.
      return `{{${variableName}}}`;
    }
  );
}
