...
/**
 * Interpolates Tiptap editor content (HTML string) with variable values.
 * This function specifically targets the <span data-type="variable-node"> elements
 * generated by the custom VariableNode extension.
 *
 * @param htmlContent The HTML string from the Tiptap editor.
 * @param variables A key-value map of variable names and their values.
 * @returns An HTML string with variables replaced by their values.
 */
export function interpolateTiptapContent(
  ...
  if (typeof DOMParser === 'undefined') {
    // Basic fallback for non-browser environments (like tests)
    return htmlContent.replace(/\{\{([a-zA-Z0-9_]+)\}\}/g, (_, varName) => variables[varName] || `{{${varName}}}`);
  }

  const parser = new DOMParser();
  const doc = parser.parseFromString(htmlContent, 'text/html');
  
  // The custom node is a span with a specific data-name attribute.
  const variableNodes = doc.querySelectorAll('span[data-name]');

  variableNodes.forEach(node => {
    const varName = node.getAttribute('data-name');
    if (varName && variables[varName] !== undefined) {
      // Replace the node's content with the variable's value
      node.textContent = variables[varName];
      // Optional: Add a class to indicate it's been replaced
      node.classList.add('interpolated');
    } else if (varName) {
      // Keep original placeholder if no value is provided
...
/**
 * Converts Tiptap editor content (HTML) to a plain text string,
 * removing any HTML tags and keeping only the text content.
 *
 * @param htmlContent The HTML string from the Tiptap editor.
 * @returns A plain text string with the content of the HTML.
 */
